...
contract CoopySwapAMMManager {
    ...
    function initializeLP(
        ...
        (address firstToken, address secondToken) = token1 < token2 ? (token1, token2) : (token2, token1);
        
        // Hash the token addresses 
        bytes memory pairBytes = abi.encodePacked(firstToken, secondToken);
        bytes32 tokenPairHash = keccak256(pairBytes);

        // Check for an existing pair
        bool pairAlreadyExists = existingPairs[firstToken][secondToken] == address(0);

        if (pairAlreadyExists) {
          revert PairAlreadyExistsError('That liquidity pool already exists!', existingPairs[firstToken][secondToken]);
        }

        // Initialize LP and store its new address        
        address newLP = address(new CoopySwapLiquidityPool{ salt: tokenPairHash }(firstToken, secondToken));
